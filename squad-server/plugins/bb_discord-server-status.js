...
import BB_DiscordBaseMessageUpdater from './bb_discord-base-message-updater.js';

export default class BB_DiscordServerStatus extends BB_DiscordBaseMessageUpdater {
  static get description() {
    return 'The <code>BB_DiscordServerStatus</code> plugin can be used to get the server status in Discord.';
  }
  ...
  async generateMessage() {
    ...
    let currentTime = new Date(new Date().getTime() - (this.server.layerHistory[0]?.time ||new Date().getTime()));
